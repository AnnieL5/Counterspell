...
platform_img = pygame.image.load(Wall_img)
num_platforms = 8
...
def draw_setting():
	...
	# Drawing banana
	if len(Wall_list) == num_platforms:
...
def update_monkey():
	...
	global monkey_x, monkey_y, velocity_y, monkey, monkey_img, Wall_list,score
	...
	if len(Wall_list) == num_platforms:  # Whenever platforms are all generated, play the animation
		if sprite_frame == 1:
	...
	if velocity_y < 0 and len(Wall_list) == num_platforms: #Jumping sprite
		monkey_img = pygame.image.load(f"hungry_monkey_sprites/6monkey_jump_sprite.png")
	# Detect banana collision
	if monkey.colliderect(banana) and len(Wall_list) == num_platforms:
		Wall_list = []
...
def generate_platforms():
	...
	current_platform_count = len(Wall_list)
	while current_platform_count < num_platforms:
		platform_x = (random.randint(0, SCREEN_WIDTH - Wall_width)) #random.randint(a,b) generate a random integer between a and b
		...
		Wall_list.append(pygame.Rect(platform_x, platform_y, Wall_width, Wall_height))

		current_platform_count += 1

def reset_variables():
	"""Every time the game_loop is rerun, reset relevant variables"""
	global frames_left, score, Wall_list, monkey_x, monkey_y
